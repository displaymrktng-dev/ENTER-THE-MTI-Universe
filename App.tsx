          ...
          >
            ...
            {!submitted ? (
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col gap-6">
                ...
                <a
                  href="https://mtiradio.substack.com/s/the-vault" target="_blank" rel="noopener noreferrer"
...
const getPlatformIcon = (platform: string) => {
  switch (platform) {
    case 'YT':    return <Youtube size={13} />;
    case 'FB':    return <Facebook size={13} />;
    case 'IG':    return <Instagram size={13} />;
    case 'SS':    return <BookOpen size={13} />;
...
const CollectiveSphere = () => (
  <div className="collective-sphere-wrap" aria-hidden="true">
    <svg className="collective-sphere" viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
      <defs>
        <radialGradient id="sg" cx="38%" cy="35%" r="60%">
          <stop offset="0%"   stopColor="#2a2a00" stopOpacity="0.9" />
          <stop offset="50%"  stopColor="#111100" stopOpacity="0.7" />
          <stop offset="100%" stopColor="#000000" stopOpacity="0.95" />
        </radialGradient>
        <radialGradient id="shine" cx="30%" cy="28%" r="45%">
          <stop offset="0%"   stopColor="#d4ff00" stopOpacity="0.13" />
          <stop offset="100%" stopColor="#d4ff00" stopOpacity="0" />
        </radialGradient>
        <clipPath id="sc"><circle cx="100" cy="100" r="90" /></clipPath>
      </defs>
      <circle cx="100" cy="100" r="90" fill="url(#sg)" />
      {[30,55,80,105,130,155,170].map((y, i) => {
        const dy = y - 100;
        const rx = Math.sqrt(Math.max(0, 8100 - dy * dy));
        return rx > 2 ? <ellipse key={i} cx="100" cy={y} rx={rx} ry={rx * 0.28} fill="none" stroke="#d4ff00" strokeWidth="0.45" strokeOpacity="0.2" clipPath="url(#sc)" /> : null;
      })}
      {[0,30,60,90,120,150].map((a, i) => (
        <ellipse key={i} cx="100" cy="100" rx={90 * Math.abs(Math.cos(a * Math.PI / 180))} ry="90"
          fill="none" stroke="#d4ff00" strokeWidth="0.4" strokeOpacity="0.15"
          clipPath="url(#sc)" transform={`rotate(${a}, 100, 100)`} />
      ))}
      <ellipse cx="100" cy="100" rx="112" ry="24" fill="none" stroke="#d4ff00"
        strokeWidth="0.9" strokeOpacity="0.35" strokeDasharray="5 7" transform="rotate(-18,100,100)" />
      <circle cx="100" cy="100" r="90" fill="url(#shine)" />
      <circle cx="100" cy="100" r="90" fill="none" stroke="#d4ff00" strokeWidth="1.2" strokeOpacity="0.28" />
      <circle cx="68"  cy="72"  r="2.5" fill="#d4ff00" opacity="0.8" />
      <circle cx="130" cy="88"  r="2"   fill="#00d4ff" opacity="0.7" />
      <circle cx="95"  cy="130" r="2"   fill="#d4ff00" opacity="0.6" />
      <circle cx="148" cy="118" r="1.5" fill="#ffc800" opacity="0.7" />
      <circle cx="55"  cy="115" r="1.5" fill="#00d4ff" opacity="0.6" />
      <line x1="68" y1="72" x2="130" y2="88"  stroke="#d4ff00" strokeWidth="0.5" strokeOpacity="0.22" clipPath="url(#sc)" />
      <line x1="130" y1="88" x2="148" y2="118" stroke="#d4ff00" strokeWidth="0.5" strokeOpacity="0.22" clipPath="url(#sc)" />
      <line x1="55" y1="115" x2="95"  y2="130" stroke="#00d4ff" strokeWidth="0.5" strokeOpacity="0.22" clipPath="url(#sc)" />
    </svg>
  </div>
);

// ─── PLANET PANEL (expandable content for non-Collective nodes) ──────────────
const PlanetPanel = ({ node }: { node: NodeData }) => {
  ...
  return (
    <div className="flex flex-col gap-0">
      ...
      <p className="font-mono text-[0.6rem] tracking-[0.08em] text-[var(--color-ghost)] leading-[1.8] uppercase mb-5">{p.bio}</p>
      ...
      {p.links && p.links.length > 0 && (
        <div className="flex flex-col gap-[1px] bg-[var(--color-wire)] mb-4">
          {p.links.map((link: PlanetLink, i: number) => (
            ...
            >
              <span className="flex items-center gap-2 text-[var(--color-volt)] w-12 shrink-0">
                {getPlatformIcon(link.platform || '')}
                <span className="font-mono text-[0.45rem] tracking-[0.2em] uppercase text-[var(--color-mist)]">{link.platform}</span>
              </span>
              <span className="font-mono text-[0.6rem] tracking-[0.1em] text-[var(--color-white)] uppercase group-hover:text-[var(--color-volt)] transition-colors flex-1">
                {link.label}
              </span>
              <span className="font-mono text-[0.75rem] text-[var(--color-volt)] opacity-0 group-hover:opacity-100 transition-opacity">↗</span>
            </a>
          ))}
        </div>
      )}
      ...
      {p.mediaEmbed && (
        <div className="mb-4">
          ...
          <div className="relative w-full bg-[var(--color-void)] border border-[var(--color-wire)]" style={{ paddingBottom: '56.25%' }}>
            <iframe src={p.mediaEmbed.url}
              className="absolute inset-0 w-full h-full"
              allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
              allowFullScreen title={p.mediaEmbed.label}
              onClick={(e) => e.stopPropagation()}
            />
            {p.mediaEmbed.url.includes('placeholder') && (
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-[var(--color-void)]">
                <Play size={22} className="text-[var(--color-volt)] mb-2 opacity-35" />
      ...
      {p.contact && (
        ...
        >
          <div>
            ...
            <div className="font-mono text-[0.62rem] tracking-[0.1em] text-[var(--color-white)] uppercase group-hover:text-[var(--color-volt)] transition-colors">{p.contact.value}</div>
          </div>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-[1px] bg-[var(--color-wire)]">
      {node.subBrands?.map((brand) => (
        <a key={brand.id} href={brand.url} target="_blank" rel="noopener noreferrer"
          className={`brand-tile group ${brand.id === 'tse' ? 'sm:col-span-2' : ''}`}
          onClick={(e) => e.stopPropagation()}
        >
          <div className="flex items-center gap-2 mb-1">
            <span style={{ color: brand.color }}>{getPlatformIcon(brand.platform)}</span>
            <span className="font-mono text-[0.6rem] tracking-[0.25em] uppercase" style={{ color: brand.color }}>{brand.platform}</span>
          </div>
          ...
          <div className="font-mono text-[0.62rem] tracking-[0.1em] text-[var(--color-ghost)] leading-[1.6] uppercase mt-1">
            {brand.description}
          </div>
  ...
  if (node.isExpandable) {
    return (
      ...
      >
        ...
        <div className="flex items-center justify-between mb-auto" style={{ position: 'relative', zIndex: 1 }}>
        ...
        <div className={`transition-opacity duration-200 ${isExpanded ? 'opacity-0 h-0 overflow-hidden' : 'opacity-100'}`} style={{ position: 'relative', zIndex: 1 }}>
          <div className="font-display text-[clamp(1.4rem,2.5vw,2.2rem)] leading-none tracking-[0.04em] text-[var(--color-white)] uppercase mt-8">
            {node.title}
          </div>
        ...
        <div
          className={`overflow-hidden transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${isExpanded ? 'max-h-[1200px] opacity-100 mt-4' : 'max-h-0 opacity-0'}`}
          style={{ position: 'relative', zIndex: 1 }}
        >
          {node.planet ? <PlanetPanel node={node} /> : node.subBrands ? <CollectivePanel node={node} /> : null}
        </div>
      </div>
    );
  }
